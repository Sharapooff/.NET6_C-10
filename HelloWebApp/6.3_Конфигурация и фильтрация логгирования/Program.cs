...
// Например, по умолчанию в проект по типу Empty добавляется файл appsettings.json, который предназначен для конфигурации проекта и
// в том числе может определять и по умолчанию уже содержит конфигурацию логгера
// Узел "Logging" задает настройки логгирования. Далее узел LogLevel задает минимальный уровень логгирования для двух категорий. Категория "Default" представляет
// универсальное применение ко всем категориям. То есть по умолчанию будут логгироваться все сообщения уровня Information. Но для логгера с категорией
// "Microsoft.AspNetCore" будут логгироваться только сообщения уровня Warning.
...
//          }
//        }
//Можно определять фильтры сразу и в коде C#, и в файле конфигурации. При создании логгера объект ILoggerFactory выбирает и применяет одно правило для каждого провайдера. 
...
//Из выбранных на предыдущем шаге правил выбирается правило, которое имеет наиболее длинное соответствие имени категории. Если такого нет, то выбираются все правила, 
//            общие для всех категорий.
//Если на предыдущем шаге выбрано несколько правил, из них выбирается самое последнее(Порядковый номер правила соответствует его порядку определения в файле конфигурации 
//    или в коде.Причем правила из файла конфигурации предшествуют правилам, определенным в коде).
//Если на предыдущем шаге не выбрано никаких правил, то применяется настройка MinimumLevel.Данную опцию также можно определить в коде, например, при настройке фабрики логгера:
